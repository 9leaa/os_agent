...
- [ ] **A0：本地 Pi 可用**
  - [x] 固定并安装 Pi 0.86.1，记录已测 Node.js 24.9.0
  - [ ] 固定并验证模型
  - [x] 提供项目安装和安全启动命令
  - [ ] 跑通终端多轮对话
  - [ ] 限制 Agent 只能访问项目测试目录
...
首次进入后执行 `/login` 配置模型服务，再用 `/model` 选择模型。配置、凭证和会话只保存在项目内已忽略的 `.runtime/`；不要把密钥写进命令、代码或 Git。

当前启动入口禁用了全部工具、扩展、Skills、模板和上级上下文，只用于先验收模型登录和多轮对话，不能读写文件或执行 Shell。受控文件工具完成前，不要直接使用裸 `pi` 代替该入口。

Python 测试命令不启动 Pi、不调用模型，也不操作桌面。模型凭证、VM、系统镜像和原始桌面证据不得提交到仓库。
